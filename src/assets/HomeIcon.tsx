// src/components/icons/HeartMapIcon.tsx
import React from 'react';

interface HeartMapIconProps {
  size?: number | string;       // 控制图标大小，默认 24
  stroke?: string;              // 描边颜色，默认 currentColor（继承父级文字颜色）
  strokeWidth?: number;         // 描边宽度，默认 1
  className?: string;           // 额外 CSS 类名
}

const HeartMapIcon: React.FC<HeartMapIconProps> = ({
  size = 24,
  stroke = 'currentColor',
  strokeWidth = 2,
  className = '',
  ...
  <svg
    ...
    fill="none" // 所有填充由 path 的 fill 控制，这里设为 none
  >
    <path
